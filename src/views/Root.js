...
const UnauthenticationApp = ({ handleSignIn, loginError }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  return (
    <form
      onSubmit={handleSubmit(handleSignIn, loginError)}
  ...
  return (
    <Router>
      <ThemeProvider theme={theme}>
        <GlobalStyle />
        {user ? <AuthenticationApp /> : <UnauthenticationApp loginError={error} handleSignIn={handleSignIn} />}
      </ThemeProvider>
    </Router>
  );
};
